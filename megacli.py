"""
megacli is an abstraction layer to the IBM MegaCli utility.

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
WARNING: Some methods are destructive and do NOT prompt for confirmation
before executing. Understand what you are doing before you call
instance methods.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
"""
...
class Adapter:
    """
    Adapter class for creating instances of any adapters found.
    Obtain a list of Adapter instances by calling adp_list()
    """
    ...
    def load(self, adapter_id):
        """
        Function for polling system for adapter attributes

        @param adapter_id: Integer - adapter ID to load
        """
        try:
...
class Enclosure:
    """
    Enclosure class for creating instances of any enclosures found.
    Obtain a list of Enclosure instances by calling enc_list()
    """
    ...
    def load_from_text(self, input):
        """
        Function for obtaining and setting instance attributes
...
class PhysicalDevice:
    """
    PhysicalDevice class for creating instances of any physical drives found.
    Obtain a list of PhysicalDrive instances by calling pd_list()
    """
    def __init__(self):
        self.adapter_id = 0
        self.enclosure_id = 0
    ...
    def led_on(self):
        """
        Instance method which attempts to turn on locator LED
        """
        try:
    ...
    def led_off(self):
        """
        Instance method which attempts to off turn on locator LED
        """
        try:
    ...
    def is_configured(self):
        """
        Returns True if drive's firmware state indicates it is unconfigured and
        in good health. This indicates the drive can be added to a new/existing
        Virtual Drive group.
        """
    ...
    def reload(self):
        """
        Reloads attributes of self. Typically used after using an instance
...
class VirtualDrive:
    """
    VirtualDrive class for creating instances of any virtual disks found.
    Obtain a list of VirtualDrive instances by calling vd_list()
    """
    def __init__(self):
    ...
    def convert_raid_level(self, raid_level):
        """
        Instance method for converting current raid level to a new one
        Currently supported levels are 0, 1, 5

        @param raid_level: Integer 0, 1, or 5
        """
    ...
    def set_wb(self):
        """
        Instance method for setting Write-back cache mode on
        """
    ...
    def setNoCacheBadBBU(self):
        """
        Instance method for disabling write cache if BBU is deemed bad
        """
    ...
    def destroy(self):
        """
        Instance method for removing virtual disk
        WARNING: this method is destructive, irreversible, and does NOT prompt
        for confirmation.
        """
    ...
    def view_rebuild_progress(self):
        """
        Instance method for getting current rebuild status
        """
    ...
    def extend(self, drives):
        """
        Instance method for extending virtual disk with unconfigured physical disks

        @param drives: List in the form of ['E:S', '...']
        """
...
class BBU():
    """
    BBU class for creating instances of any BBU found.
    Obtain a list of BBU instances by calling bbu_list()
    """
    def __init__(self):
        self.battery_type = ''
        self.voltage = ''
...
def adp_list():
    """
    Function returns list of Adapter instances found
    """
    try:
...
def enc_list(adapter_id):
    """
    Function returns list of Enclosure instances found

    @param adapter_id: Integer representing adapter id to search
    """
    try:
...
def pd_list(adapter_id):
    """
    Function returns list of PhysicalDrive instances found
...
def vd_list(adapter_id):
    """
    Function returns list of VirtualDrive instances found
...
def bbu_list(adapter_id):
    """
    Function returns list of BBU instances found
...
def createvd(raid_level, drives, adapter):
    """
    Function for creating a new virtual disk

    @param drives: List in form of ['E:S', '...'], where E=Enclosure, S=Slot
                    example: ['252:0', '252:1', '252:2', '252:3']

    @param adapter: Integer adapter to create VD on
                    example: 0
    """
    if not type(drives) == types.ListType:
        raise TypeError("drives must be in list format ['E:S', 'E:S', '...']")
...
def pd_list_unconfigured(drives=None):
    """
    Function returns List of PhysicalDevice instances that are unconfigured
    and in good health.  Optionally, a list of PhysicalDevice instances can
...
def pd_list_configured():
    """
    Function returns list of PhysicalDevice instances that are already configured.
    That is to say, they are used within a Virtual Disk
...
def str2bool(str):
    """
    Function returns True if supplied string is one of 'Present', 'OK', or 'Yes'
    else, returns False
...
def megacli(args):
    """
    Wrapper function for calling MegaCli utility with supplied args
    returns stdout result of execution

    @param args: String of arguments to be passed to MegaCli binary
    """
    cmd = MEGACLI + ' ' + args
...
def decode_raid_level(description):
    """
    Maps descriptive format raid level to simple human readable format
    and vice versa.

    @param description: String such as:
     descriptive format such as 'Primary-5, Secondary-0, RAID Level Qualifier-3'
     OR, Integer such as: 0, 1, 5, 6, or 10
    """
    raid_map = {
